...
class DatabaseService {
  async ensureDb() {
    try { await fs.access(BASE_PATH); } catch { await fs.mkdir(BASE_PATH, { recursive: true }); }
    try { await fs.access(DB_PATH); } catch { await fs.writeFile(DB_PATH, JSON.stringify({ folders: [] }, null, 2)); }
  }
